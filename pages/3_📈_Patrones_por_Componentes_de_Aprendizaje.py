...
materias_disponibles_aaBim1 =  ['Metodología de la invest y tec(aaBim1)',
                        'Computación y sociedad(aaBim1)', 'Fundamentos de tecnologias(aaBim1)',
...
materias_disponibles_acdBim1 =  ['Metodología de la invest y tec(acdBim1)',
                        'Computación y sociedad(acdBim1)', 'Fundamentos de tecnologias(acdBim1)',
                        'Humanismo universidad y cultu(acdBim1)', 'Fundamentos matematicos(acdBim1)'
                        ]

materias_disponibles_acdBim2 =  [
                        'Computación y sociedad(acdBim2)', 'Fundamentos de tecnologias(acdBim2)',
                        'Humanismo universidad y cultu(acdBim2)', 'Fundamentos matematicos(acdBim2)'
                        ]

materias_disponibles_apeBim1 =  ['Computación y sociedad(apeBim1)', 'Metodología de la invest y tec(apeBim1)',                        
                        'Humanismo universidad y cultu(apeBim1)', 'Fundamentos matematicos(apeBim1)'
                        ]

materias_disponibles_apeBim2 =  ['Computación y sociedad(apeBim2)', 'Fundamentos de tecnologias(apeBim2)',
                                 'Algoritmos y resolución de pro(apeBim2)'
                        ]
...
if bimestre_elegido == "Primer Bimestre":
    # Widget principal para elegir el bimestre
    componentes = st.selectbox("Selecciona el componente especifico:", [
                                                                        "Aprendizaje Autonomo Bim1",
                                                                        "Aprendizaje en contacto con el docente Bim1",
                                                                        "Aprendizaje practico experimental Bim1"])
elif bimestre_elegido == "Segundo Bimestre":
    componentes = st.selectbox("Selecciona el componente especifico:", [
                                                                        "Aprendizaje Autonomo Bim2",
                                                                        "Aprendizaje en contacto con el docente Bim2",
                                                                        "Aprendizaje practico experimental Bim2"])
else:
...
# Widgets para elegir las materias según el bimestre seleccionado
    ...
    if bimestre_elegido == "Primer Bimestre":
        if componentes == "Aprendizaje Autonomo Bim1":
            # Widget principal para elegir el bimestre
            materia_x = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_aaBim1, key="materia_x1")
            materia_y = st.selectbox("Selecciona la materia para el eje Y:", materias_disponibles_aaBim1, key="materia_y1")
            
        elif componentes == "Aprendizaje en contacto con el docente Bim1":
            materia_x = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_acdBim1, key="materia_x1")
            materia_y = st.selectbox("Selecciona la materia para el eje Y:", materias_disponibles_acdBim1, key="materia_y1")  
            
        elif componentes == "Aprendizaje practico experimental Bim1":
            materia_x = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_apeBim1, key="materia_x1")
            materia_y = st.selectbox("Selecciona la materia para el eje Y:", materias_disponibles_apeBim1, key="materia_y1")   
                     
    elif bimestre_elegido == "Segundo Bimestre":
        if componentes == "Aprendizaje Autonomo Bim2":
            # Widget principal para elegir el bimestre
            materia_x = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_aaBim2, key="materia_x1")
            materia_y = st.selectbox("Selecciona la materia para el eje Y:", materias_disponibles_aaBim2, key="materia_y1")
            
        elif componentes == "Aprendizaje en contacto con el docente Bim2":
            materia_x = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_acdBim2, key="materia_x1")
            materia_y = st.selectbox("Selecciona la materia para el eje Y:", materias_disponibles_acdBim2, key="materia_y1")  
            
        elif componentes == "Aprendizaje practico experimental Bim2":
            materia_x = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_apeBim2, key="materia_x1")
            materia_y = st.selectbox("Selecciona la materia para el eje Y:", materias_disponibles_apeBim2, key="materia_y1")   
    else:
        st.error("Error inesperado")
...
# Widgets para elegir las materias según el bimestre seleccionado
    if bimestre_elegido == "Primer Bimestre":
        if componentes == "Aprendizaje Autonomo Bim1":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_aaBim1, key="materia_y")
            
        elif componentes == "Aprendizaje en contacto con el docente Bim1":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_acdBim1, key="materia_y")
            
        elif componentes == "Aprendizaje practico experimental Bim1":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_apeBim1, key="materia_y")
            
    elif bimestre_elegido == "Segundo Bimestre":
        if componentes == "Aprendizaje Autonomo Bim2":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_aaBim2, key="materia_y")
    
        elif componentes == "Aprendizaje en contacto con el docente Bim2":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_acdBim2, key="materia_y")
            
        elif componentes == "Aprendizaje practico experimental Bim2":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_apeBim2, key="materia_y")
...
with col2:
    ...
    # Crear la figura dinámicamente
    fig1 = px.scatter(filtered_data, x=materia_x, y=materia_y,
                color="Cluster",
                template="simple_white",
                labels={materia_x: "Porcentaje de beca",  
                        materia_y: materia_y.capitalize(), 
                        "cluster": "Desempeño académico"},
                title=f"Relación entre las materias de {materia_y.capitalize()}<br> y el porcentaje de beca")
...
# Widgets para elegir las materias según el bimestre seleccionado
    if bimestre_elegido == "Primer Bimestre":
        if componentes == "Aprendizaje Autonomo Bim1":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_aaBim1, key="materia_y2")
            
        elif componentes == "Aprendizaje en contacto con el docente Bim1":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_acdBim1, key="materia_y2")
            
        elif componentes == "Aprendizaje practico experimental Bim1":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_apeBim1, key="materia_y2")
            
    elif bimestre_elegido == "Segundo Bimestre":
        if componentes == "Aprendizaje Autonomo Bim2":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_aaBim2, key="materia_y2")
    
        elif componentes == "Aprendizaje en contacto con el docente Bim2":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_acdBim2, key="materia_y2")
            
        elif componentes == "Aprendizaje practico experimental Bim2":
            materia_y = st.selectbox("Selecciona la materia para el eje X:", materias_disponibles_apeBim2, key="materia_y2")
...
with col2:
    ...
    # Crear la figura dinámicamente
    fig2 = px.scatter(filtered_data, x=materia_x, y=materia_y,
                color="Cluster",
                template="simple_white",
                labels={materia_x: "Porcentaje de Discapacidad",  
                        materia_y: materia_y.capitalize(), 
                        "cluster": "Desempeño académico"},
                title=f"Relación entre las materias de {materia_y.capitalize()}<br> y el porcentaje de discapacidad")
...
# Descargar cada gráfica y añadirla al PDF
    ...
    st.download_button(
        label="Descargar PDF",
        data=pdf_bytes,
        file_name="dashboard_clustering_académico.pdf",
        key="download_pdf"
    )
